...
pheatmap(signal_values, 
         ...
         main = "Differential Peak Signal Across Heat and Control Samples",
         show_rownames = FALSE,
         legend = TRUE)
...
ggplot(df, aes(x = x, y = y, color = factor(case_when(x > y ~ "x", x < y ~ "y", TRUE ~ "equal")))) +
  ...
  ylab("Heat peaks") +
  xlim(0, 100) +
  ggtitle("Comparison of Control and Heat-stress peaks") +
  scale_color_manual(values = c("blue", "red"), labels = c("Control", "Heat")) +
  labs(color = "Sample Type")

# Loading in bam files
bam_files <- list.files(pattern = "sort.bam$", full.names = TRUE)

# Get the column names of the coefficients in qlf
coef_names <- colnames(qlf$table)[2:length(colnames(qlf$table))]
...
barplot(heat_total_rowMeans[,3], main="Total Peaks for Heat Stress", xlab="Genes", ylab="Number of Peaks", las=2, names.arg = rep("", nrow(heat_total_rowSums)))
...
# Add legend
legend("bottomright", legend = c("P-value < 0.05", "P-value >= 0.05"), 
...
# Create new volcano plot looking at top 20 peaks ad genes associated with them
