...
before <- counts_lizard[counts_lizard$which %in% c("not-matched" , "total", "Lizard"), ]
After <- counts_lizard[counts_lizard$which %in% c("Human" , "prokaryotes", "Lizard"), ]

sample_level1 <- factor(before$which, level=c("total", "Lizard", "not-matched" ))
...
kruskal.test( before$counts ~ before$Sample )
kruskal.test( before$counts ~ before$which )
dunnTest(before$counts ~ before$which)

lizard_con <- before[before$Treatment != "HeatStress", ]
kruskal.test( lizard_con$counts ~ lizard_con$which )
dunnTest(lizard_con$counts ~ lizard_con$which)

lizard_heat <- before[before$Treatment != "Control", ]
kruskal.test( lizard_heat$counts ~ lizard_heat$which )
dunnTest(lizard_heat$counts ~ lizard_heat$which)
```


```{r}
#normality test
shapiro.test(counts_lizard$log)
#significance test
kruskal.test( counts_lizard$log ~ counts_lizard$Treatment)
kruskal.test( counts_lizard$log ~ counts_lizard$Sample )
```


```{r}
counts_heat <- counts_lizard[counts_lizard$Treatment != "Control", ]

kruskal.test(counts_heat$log ~ counts_heat$which )
library(FSA)
dunnTest(counts_heat$log ~ counts_heat$which )
kruskal.test( counts_heat$log ~ counts_heat$Sample )
...
```{r}
counts_lizard_con <- counts_lizard[counts_lizard$Treatment != "HeatStress", ]
kruskal.test( counts_lizard_con$log ~ counts_lizard_con$which )
dunnTest(counts_lizard_con$log ~ counts_lizard_con$which )
kruskal.test( counts_lizard_con$log ~ counts_lizard_con$Sample )
